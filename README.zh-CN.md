...
**AI 经常给出类型错误的回应。** 你想被听见，它开始提建议；你只想轻微润色，它重写了整段话；代码和资料都没有检查，它已经说得很确定。

Calm Agent 是一个可移植的回应策略层。它先判断当前时刻需要什么，再决定该怎样表达。

- **更个人化的交流：** 根据你的边界、纠正和上下文作答，减少到处套用同一种友好话术。
...
公开项目不应包含原始对话、已填写的测试答案、个人评分历史或本地运行日志。只发布可复用指令、空白测试和合成示例。
...
- 提示词层面的回应选择仍然带有概率性，细微的情感依附、范围和强度变化仍可能破坏精确改写。
- “下一轮影响”是一种交互启发式方法，可以减少不必要的对话负担，但无法可靠推断用户没有说出的偏好。
- 当前价格、版本、法律、模型可用性和研究元数据需要实时核验；Skill 无法创造宿主模型本来没有的访问能力。
- 尚未填写的公共测试集不能支持跨模型性能或人类偏好结论。
- 原生 Skill 兼容只说明宿主能够发现当前包结构，不能证明不同宿主具有相同的指令遵循程度或一致输出。
- 真实持久记忆会改变记忆测试的正确答案；能力评估必须控制是否提供了存储工具。
- Calm Agent 可以引导回应选择与边界处理，但无法复现训练阶段形成的角色特性，也不能替代基础模型的推理、检索或安全系统。
- 目标模型和评判模型来自同一模型家族时，校准可能出现相关偏差。请保留人工复核，并尽量使用独立评判模型。
